...
NTSTATUS NTAPI After_ZwOpenKeyEx_Func(IN ULONG FilterIndex, IN PVOID ArgArray, IN NTSTATUS InResult, IN PULONG RetFuncArgArray)
{
...
NTSTATUS NTAPI Fake_ZwOpenKeyEx(IN ULONG CallIndex, IN PVOID ArgArray, IN PULONG ret_func, IN PULONG ret_arg)
{
	...
	if (ExGetPreviousMode())
	{
		...
		if (KEY_READ == DesiredAccess || KEY_WRITE == DesiredAccess)
		{
			...
			//3、启动调用后检查
			*(ULONG*)ret_func = After_ZwOpenKeyEx_Func;
		}
	}
	return result;
}